...
//	    	System.out.println(rs.getInt("reportID") + "\t");
    ...
    public void toForecastView(ActionEvent event) throws Exception {
    	...
    	if(station == null){
    		Alert alert = new Alert(AlertType.WARNING, "No weather station selected!");
    		alert.showAndWait();
    	} else {
	    	Context.getInstance().setStation(station);
	        stage = (Stage) ((Button) event.getSource()).getScene().getWindow();
	        root = FXMLLoader.load(getClass().getResource("../view/ForecastGUI2.fxml"));
	        scene = new Scene(root);
	        stage.setScene(scene);
    	}
    }
